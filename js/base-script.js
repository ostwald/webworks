...
var MOBILE_BROWSER = screen.availWidth < 667;
...
function rotate_img(el, degrees) {
    $(el).css({
        '-ms-transform': 'rotate('+ degrees + 'deg)', // IE 9 */
        '-webkit-transform': 'rotate('+ degrees + 'deg)', /* Safari */
        transform: 'rotate('+ degrees + 'deg)',
    })
}
/**
rotates el a single time at specified speed and inc
*/
function rotateAnimation (el, speed) {
    var degrees = 0;
    speed = speed || 10;
    var inc = 1;
    var speed = getRandomIntInclusive(10,15);
//    var inc = getRandomIntInclusive(100,200)/150;

    // log (el.attr('src') + ' speed: ' + speed + " inc: " + inc)
    function doRotate() {
        rotate_img(el, degrees)
        looper = setTimeout(doRotate,speed);
//        degrees++;
        degrees += inc;
        if(degrees > 359){
            clearTimeout(looper);
